	...
	namespace {
		...
		public:
			...
			double Evaluate(const SheetInterface& sheet) const override {
				double lhs_value = lhs_.get()->Evaluate(sheet);
				double rhs_value = rhs_.get()->Evaluate(sheet);
				double result = 0.0;
		...
		public:
			...
			double Evaluate(const SheetInterface& sheet) const override {
				double value = operand_.get()->Evaluate(sheet);
				double result = 0.0;
		...
		public:
			...
			// Проверяет тип данных ячейки (пустая ячейка, строка, число, ошибка) вычисляет и возвращает соответствующее значение
			double Evaluate(const SheetInterface& sheet) const override {
				Position* pos_ptr = const_cast<Position*>(cell_);

				if (!sheet.GetCell(*pos_ptr)) { // проверка и установка пустой ячейки
					const_cast<SheetInterface&>(sheet).SetCell(*pos_ptr, ""s);
				}

				const CellInterface* cell_ptr = sheet.GetCell(*pos_ptr);
				CellInterface::Value value = cell_ptr->GetValue();

				if (double* ptr = std::get_if<double>(&value)) { // проверка значения double
					return *ptr;
				}

				if (std::string* ptr = std::get_if<std::string>(&value)) { // проверка значения string
					if (ptr->empty()) {
						return 0.0; // пустая строка 0
					}

					std::smatch smatch;
					std::regex_search(*ptr, smatch, re_digits); // извлекаются цифры

					if (smatch.size()) {
						return std::stod(smatch[0]);
					}

					throw FormulaError(FormulaError::Category::Value); // строка не может быть преобразована в число
				}

				throw std::get_if<FormulaError>(&value); // если не string, не double, то значение FormulaError
			}
